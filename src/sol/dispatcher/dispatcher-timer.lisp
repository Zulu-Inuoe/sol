...
(defclass dispatcher-timer (dispatcher-object)
  ((interval
   ...
   (%priority
    :type dispatcher-priority
    :initform +priority.background+
    :initarg :priority
    :reader %priority)
   (e_tick
...
(defgeneric (setf timer-interval) (value timer))
...
(defun %timer-thread-update (&aux due-timers)
  ...
  (dolist (timer due-timers)
    ;;TODO we're just silently ignoring any error during the invoke.. do we want that?
    ;;Specifically the dispatcher might shut down before the request finishes
    ;;Should we disable the timer if that's the case?
    (ignore-errors
     (do-invoke ((dispatcher timer) :priority (%priority timer))
      (unwind-protect
