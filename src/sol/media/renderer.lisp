...
(defmacro render-with-new-target ((renderer width height &key (opacity 1.0)) &body body)
  (once-only (renderer)
    (with-gensyms (new-target successful)
      `(let ((,new-target (render-create-target ,renderer ,width ,height :opacity ,opacity))
             (,successful nil))
         (unwind-protect
              (progn
                ...
                ,new-target)
           (unless ,successful
             (render-destroy-target ,renderer ,new-target)))))))
...
(defgeneric render-draw-image (renderer image x y &key src-x src-y src-width src-height dst-width dst-height flip)
  (:documentation
   "Use `renderer' to draw `image' at coordinates `x',`y'.
`src-x', and `src-y', along with `src-width' and `src-height' allow drawing a portion of the image, instead
`dst-width' and `dst-height' apply scaling of the image width and height
`flip' is `render-flip', and indicates a flip to perform during the draw operation. (deprecated)"))
