...
(defun %cache-image-surface (path color-key
  ...
  (let ((surface (gethash key map)))
    (unless surface
      (setf surface (cons (sdl2-image:img-load (uiop:native-namestring (truename path)) ) 0))
      (unless (media:color= color-key media.colors:*transparent*)
        (let ((mapped-color
               (sdl2:sdl-map-rgba
                (cffi:foreign-slot-value (car surface) '(:struct sdl2:sdl-surface) 'sdl2:format)
                (media:r color-key)
                (media:g color-key)
                (media:b color-key)
                (media:a color-key))))
          (sdl2:sdl-set-color-key (car surface) sdl2:+sdl-true+ mapped-color)))
...
(defun %uncache-image-surface (path color-key
  ...
  (let ((surface (gethash key map)))
    ...
    (when (zerop (cdr surface))
      ...
      (sdl2:sdl-free-surface (car surface))
      (remhash key map)))
  (values))
