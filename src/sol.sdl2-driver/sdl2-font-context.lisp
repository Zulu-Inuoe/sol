  ...
  (with-output-to-string (str)
    (with-open-file (stream path)
      (loop
         ...
         :while line
         :doing
         (write-line #+windows(string-right-trim '(#\return) line) #-windows line str)))))
...
(defun %load-font-file (context path)
  (let ((font (sdl2-ttf:ttf-open-font (namestring path) 8)))
    (setf (gethash path (%loaded-font-files context)) font)
    font))

(defun %font-matches (font family style)
  (and (string-equal
        (sdl2-ttf:ttf-font-face-family-name font)
        family)
       (string-equal
        (sdl2-ttf:ttf-font-face-style-name font)
        style)))
  ...
  (let ((ttf-font
    ...
    (unless ttf-font
      (let ((font-path (%find-font-file context family style)))
        ...
        (setf ttf-font
              (sdl2-ttf:ttf-open-font
               (namestring font-path)
               point-size))
        (sdl2-ttf:ttf-set-font-hinting ttf-font sdl2-ttf:+ttf-hinting-light+)
        (sdl2-ttf:ttf-set-font-style
         ttf-font
...
(defun %font-properties-to-bitmask (bold italic underline strikethrough)
  (logior
   (if bold sdl2-ttf:+ttf-style-bold+ 0)
   (if italic sdl2-ttf:+ttf-style-italic+ 0)
   (if underline sdl2-ttf:+ttf-style-underline+ 0)
   (if strikethrough sdl2-ttf:+ttf-style-strikethrough+ 0)))
